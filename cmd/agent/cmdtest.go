...
// runTest 实现 `topus-agent test`：连 server 调 health.Ping 做链路预检。
// 给 --ca 则单向 TLS（用 CA 验 server 防中间人），否则明文（骨架期）。
...
func runTest(args []string) {
	...
	// 1 按是否给 --ca 选择传输凭证：给则用 CA 验 server（单向 TLS），否则明文。
	...
	// 3 建连（grpc.NewClient 惰性连接，首个 RPC 才真拨号）。
	...
	// 4 调 Ping。失败 = 链路不通/TLS 验证失败/server 异常 → 退 1。
